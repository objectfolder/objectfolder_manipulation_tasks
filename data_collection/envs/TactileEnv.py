...
from experiments.tasks.DataTask import DataTask
...
class TactileEnv(BaseEnv):
    ...
    def find_best_contact(self):
        ...
        self.robot.go(_pos=[objPosLast[0]-0.1, objPosLast[1], self.robot.pos[2]])
        for _ in range(100):
            p.stepSimulation()
        self.robot.go(_pos=[objPosLast[0]-0.1, objPosLast[1], objPosLast[2] + self.contact_height])
        for _ in range(100):
            p.stepSimulation()
        ...
        while True:
            ...
            self.robot.go(_pos=[initial_position, objPosLast[1], objPosLast[2] + self.contact_height])
            p.stepSimulation()
    ...
    def reset_robot(self):
        ...
        self.robot.init_robot()
        objPosLast, objOriLast, _ = utils.get_object_pose(p, self.simulator.objID)
        self.robot.go(_pos=[objPosLast[0]-0.2, objPosLast[1], self.robot.pos[2]])
        for _ in range(100):
            p.stepSimulation()
        self.robot.go(_pos=[objPosLast[0]-0.2, objPosLast[1], objPosLast[2] + self.contact_height])
        for _ in range(100):
            p.stepSimulation()
        initial_position = objPosLast[0]-self.contact_location
        self.robot.go(_pos=[initial_position, objPosLast[1], objPosLast[2] + self.contact_height])
        ...
        self.pos_x_limit = np.array([initial_position, initial_position + self.config.get('pos_x_limit')])
    ...
    def reset_object(self):
        ...
        d_ori_x = np.random.uniform(-0.02, 0.02)
        d_ori_y = np.random.uniform(-0.02, 0.02)
        d_ori_z = np.random.uniform(-0.02, 0.02)
        ori = [ori[0] + d_ori_x, ori[1] + d_ori_y, ori[2] + d_ori_z]
        self.simulator.objStartOrientation = p.getQuaternionFromEuler(ori)
        self.contact_height = np.random.uniform(self.config["contact_height"][0], self.config["contact_height"][1])
    ...
    def reset_robot_to(self, state):
        ...
        pos = self.robot.pos
        ori = [state, self.robot.ori[1], self.robot.ori[2]]
        if self.check_limits(pos, ori):
            self.robot.go(_pos=pos, _ori=ori)
            for _ in range(100):
                p.stepSimulation()
        else:
            print("Initial state not available!!!")
    ...
    def check_limits(self, pos, ori):
        ori_x_good = False
        print(ori, self.ori_x_limit[0])
        if ori[0] >= self.ori_x_limit[0] and ori[0] <= self.ori_x_limit[1]:
            ori_x_good = True 
        return ori_x_good
    ...
    def step(self, action=0):
        ...
        # actions = self.mapping[action]
        dpos= 0 #actions["dpos"]
        dori= action #s['dori']
        self.current_step += 1
        pos = self.robot.pos # + np.array(dpos)
