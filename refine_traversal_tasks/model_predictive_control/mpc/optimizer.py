import abc
import numpy as np
from perceptual_metrics.mpc.utils import (
    ObservationList,
    write_moviepy_gif,
...
class Optimizer(metaclass=abc.ABCMeta):
    def log_best_plans(self, filename, vis_preds, goal, scores, fps=5):
        ...
        if vis_preds_image[0].shape[1] > 640:
            score_image = np.concatenate(
                [
                    np.concatenate((generate_text_square(str(np.round(score, decimals=4).item())), np.zeros((3, 64, 64))), axis=-1)
                    for score in scores
                ],
                axis=-1,
            )
