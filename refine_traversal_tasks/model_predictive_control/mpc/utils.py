...
class ObservationList:
    ...
    def __init__(self, data_dict, add_time_dimension=False, image_shape=(64, 128)):
    ...
    def from_obs(cls, obs, cfg):
        ...
        return ObservationList(data_dict, image_shape=cfg.image_shape)
    ...
    def __getitem__(self, key):
        ...
        if isinstance(key, slice):
            return ObservationList({k: v[key] for k, v in self.data_dict.items()}, image_shape=self.image_shape)
        if isinstance(key, str):
            return self.data_dict[key]
        # Note that if we select only a single index, we maintain the time dimension
        return ObservationList({k: v[key][None] for k, v in self.data_dict.items()}, image_shape=self.image_shape)
...
def write_moviepy_gif(obs_list, name, fps=5):
    ...
    if not name.endswith(".gif"):
        name = f"{name}.gif"
    clip.write_gif(f"{name}", fps=fps, logger=None)
    clip.close()
