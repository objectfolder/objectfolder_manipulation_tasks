...
from model_predictive_control.mpc.agent import Agent
...
class RemoteAgentWrapper(Agent):
    ...
    def setup_socket(self):
        ...
        print("Setting server to accept client connections at {}:{}".format(self.host, self.port))
        ...
        while True:
            ...
            if d["t"] == 0:
                if self.obs_history is not None:
                    self.obs_history.append(self._agent.goal.repeat(len(self.obs_history)))
            ...
            goal_image = ObservationList(dict(rgb=d["goal_image"]),
                                         add_time_dimension=True,
                                         image_shape=image_size)
            obs_history = ObservationList(dict(rgb=d["images"] / 255.),
            ...
            action = self._agent.act(t=d["t"], obs_history=obs_history, state_obs_history=d["state"])
