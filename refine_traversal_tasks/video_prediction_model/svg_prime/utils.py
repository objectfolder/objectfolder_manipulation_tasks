...
def load_dataset(opt):
    if opt.dataset == 'smmnist':
        ...
        test_data = KTH(
                ...
                seq_len=opt.n_eval, 
                image_size=opt.image_width)
    elif opt.dataset == 'tac_refine':
        from data.obj_robot import TactileRefine
        train_data = TactileRefine(
            train=True,
            data_root=opt.data_root,
            seq_len = opt.n_past+opt.n_future,
            image_size=opt.image_width,
            croped_size=opt.croped_size,
            modality=opt.modality
        )
        test_data = TactileRefine(
            train=False,
            data_root=opt.test_root,
            seq_len = opt.n_past+opt.n_future,
            image_size=opt.image_width,
            croped_size=opt.croped_size,
            modality=opt.modality
        )
    elif opt.dataset == 'surface_trav':
        from data.obj_robot import SurfaceTraversal
        train_data = SurfaceTraversal(
            train=True,
            data_root=opt.data_root,
            seq_len = opt.n_past+opt.n_future,
            image_size=opt.image_width,
...
def normalize_data(opt, dtype, sequence):
    if opt.dataset == "perceptual_metrics" or opt.dataset == 'tac_refine' or opt.dataset == 'surface_trav' or opt.dataset == 'dynamic_push':
        return sequence
