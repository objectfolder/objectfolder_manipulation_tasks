...
class Renderer:
    def __init__(self, width, height, background, config_path):
        ...
        # use r15
        self.use_gelsight = ("r15" in config_path)
    ...
    def update_camera_pose(self, position, orientation):
        ...
        update their pose in pyrender based on pose from pybullet
        ### important ###
        call self.update_camera_pose before self.render
        """
        # position= (0.5001944720582152, position[1], position[2])
        pose = euler2matrix(angles=orientation, translation=position)
    ...
    def get_bias(self, camera_pos, camera_ori, normal_forces, object_poses):
        ...
        # print("cccccccccccccccccccc", bias)
        if np.abs(bias) > 0.02:
            bias = 0
        return bias
    ...
    def adjust_with_force(
            self, camera_pos, camera_ori, normal_forces, object_poses, shear_offset
    ):
        """
        ...
        # correct the move by shear force
        def add_shear(camera_pos, camera_ori, obj_pos, shear):
            camera_pos = np.array(camera_pos)
            obj_pos = np.array(obj_pos)
            obj_pos_new = obj_pos + shear
            return obj_pos_new
        ...
        for obj_name in normal_forces:
            ...
            if self.force_enabled:
                ...
                if obj_name in normal_forces:
                    ...
                    pixmm = 0.0495 # 0.0495 for gelsight R1.5
                    ...
                    start_point = offset
                    maximum_depth_mm = 2 + gap_bias * 1000 # 3.2
                    ...
                    err_threshold = 1000
                    recurring_times = 0
                    # print("!!!!!!!!!!!!!", cur_depth_pix, est_volume)
                    while np.abs(cur_volume - est_volume) > err_threshold:
                        recurring_times += 1
                        if recurring_times > 25:
                        ...
                        self.update_object_pose(obj_name, obj_pos_tmp, objOri)
                        cur_volume = self.get_volume() / pixmm * 1000
                        # print("!!!!!!!!!!!!!", cur_depth_pix, max_depth_pix)
                        # print("*************",cur_volume, est_volume)
                    offset = min(cur_depth_pix * pixmm / 1000, 0.002 + gap_bias)
                    # print("aaaaaaaaaaaaaaaa",offset, cur_depth_pix * pixmm / 1000, 0.002 + gap_bias)
                    self.vol = cur_volume
                # Calculate pose changes based on normal force
                obj_pos = get_pos(camera_pos, camera_ori, obj_pos, offset)
                # obj_pos = add_shear(camera_pos, camera_ori, obj_pos, shear_offset)
                # print("obj", obj_pos)
    ...
    def render(self, object_poses=None, normal_forces=None, noise=False, calibration=True,
        ...
        for i in range(self.nb_cam):
            ...
            if object_poses is not None and normal_forces is not None:
                ...
                # print("obj", np.array(object_poses['2_-1'][0]) - self.prev_obj)
                # print("cam", camera_pos - self.prev_cam)
                # print("delta", (np.array(object_poses['2_-1'][0]) - self.prev_obj - camera_pos + self.prev_cam)[-1])
                if '3_-1' in object_poses.keys():
                    self.prev_obj = object_poses['3_-1'][0]
                else:
                    self.prev_obj = object_poses['2_-1'][0]
                self.prev_cam = camera_pos
                
                # add shear discplacement
                shear_slope = 20
                camera_offset = - np.array(shear_forces['2_-1']) * shear_slope / 1000 * 0.0495
                # print("shear displacement", np.linalg.norm(camera_offset), camera_offset)
                
                # self.show_scene()
                self.adjust_with_force(
                    camera_pos, camera_ori, normal_forces, object_poses, camera_offset
                )
